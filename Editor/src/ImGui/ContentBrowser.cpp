		...
		if (SelectedFile != nullptr) {
			...
			if (ImGui::BeginTable("Files", columns, ImGuiTableFlags_SizingFixedFit, ImVec2(cellSize * columns, cellSize))) {
				...
				for (File& file : relevantFile->GetFiles()) {
					...
					ImVec4 tintColor = ImVec4(1.0, 1.0, 1.0, 1.0);
					if (ImGui::IsMouseHoveringRect(cursor, ImVec2(cursor.x + 64, cursor.y + 64))) {
						if (ImGui::IsMouseDown(0)) {
							tintColor = ImVec4(1.5, 1.5, 1.5, 0.3);
							s_SelectedPath = path.PathString;
						}
						else {
							tintColor = ImVec4(1.5, 1.5, 1.5, 0.5);
						}
					}
					ImGui::Image((void*)(intptr_t)icon->GetRendererID(), ImVec2(thumbnailSize, thumbnailSize), ImVec2(0, 1), ImVec2(1, 0), tintColor);
					if (isSelected) {
						ImGui::GetWindowDrawList()->AddRectFilled(cursor, ImVec2(cursor.x + 64, cursor.y + 64), IM_COL32(200, 200, 200, 50), 0.1f, 0x0F);
					}
					float currentWidth = ImGui::GetColumnWidth(i);
					ImGui::SetCursorPosX(ImGui::GetCursorPosX() + (64 - ImGui::CalcTextSize(file.GetName().c_str()).x) / 2.f);
